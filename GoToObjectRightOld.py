...
lower_Blue = np.array([100, 100, 146])
...
def doMove(direction):
    ...
    if (direction == 'SLOWFORWARD') :
        target_linear_velocity = check_linear_limit_velocity(target_linear_velocity + 0.1)
...
while(cap.isOpened()):
    ...
    if (radius > 0):
        ...
        # Close align object on the right of the screen to move acros
        if (step == 1):
            if (x <= frameWidth-140):
                print (x)
                doMove('L')
    ...
    # As long as no object in the middel of camera, go forward with camera view on the right.
    if (step == 3) :
        print("step3")
        if (radius >70) :
